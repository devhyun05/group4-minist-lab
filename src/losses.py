...
def cross_entropy_loss(y_pred, y_true):
    ...
    if y_pred.ndim == 1:
      y_true = y_true.reshape(1, y_true.batch_size)
      y_pred = y_pred.reshape(1, y_pred.batch_size)

    batch_size = y_pred.shape[0]
    return -np.sum(np.log(np.clip(y_pred[np.arange(batch_size), y_true], 1e-7, 1.0))) / batch_size